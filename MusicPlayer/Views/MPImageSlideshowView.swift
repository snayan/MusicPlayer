...
class MPImageSlideshowView: UIView {
    ...
    fileprivate var slideshowImages: [UIImageView] = []
    ...
    fileprivate func setSlideshowImage() {
        let originImageViews = data.map{UIImageView(image: $0)}
        var copyImageViews: [UIImageView] = []
        if circular {
            copyImageViews = data.map{UIImageView(image: $0)}
        }
